...
$$
and the coefficient of $|a_1\cdots a_t\rangle = |a\rangle$, which can be computed when $j = a$\footnote{$\frac{1}{2^t}\sum_{j=0}^{2^t-1}\sum_{k=0}^{2^t-1}e^{-\frac{2 \pi i (a-j) k}{2^m}}e^{2 \pi i \delta k}|j\rangle \hspace{0.5cm} \underset{j=a}{\longrightarrow} \hspace{0.5cm} \frac{1}{2^t}\sum_{k=0}^{2^t-1}e^{2 \pi i \delta k}|a\rangle$}, in the above is the geometric series
$$
...
\left| \frac{1}{2^t}\left( \frac{1-(e^{2 \pi i \delta})^{2^t}}{1-e^{2 \pi i \delta}}\right)\right|^2 \geq \left( \frac{1}{2^t} \left( \frac{4\delta 2^t}{2 \pi \delta}\right)\right) ^2 = \frac{4}{\pi^2}.
$$
...
\paragraph{Lemma \textit{(Amplitude Amplification)}} Let \textbf{A} be some quantum procedure and \textbf{S} some set of basis states on the algorithm's output space. Suppose that the probability that measuring the state $\textbf{A}|0\rangle$ gives a basis state in \textbf{S} is at least $p$. Then there exist another procedure \textbf{B}, which invokes \textbf{A} and $\textbf{A}^{-1}$ $\mathbb{O}\left( \frac{1}{\sqrt{p}}\right)$ many times (also called "\textit{round of amplitude amplification}"), such that the probability of measuring the state $\textbf{B}|0\rangle$ gives a basis in \textbf{S} is at least $\frac{9}{10}$. If, in contrast, the probability of obtaining a basis state in \textbf{S} when measuring $\textbf{A}|0\rangle$ was $0$, this probability will still be $0$ when measuring $\textbf{B}|0\rangle$. 
