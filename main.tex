...
Pr\left(Y_{t} = y_t|Y_{t-1} = y_{t-1},\cdots,Y_1 = y_1\right) = Pr\left(Y_{t} = y_t|Y_{t-1} = y_{t-1}\right) \eqqcolon P_n\left(y_{t-1},y_t\right)$$
with
$$ 
...
P_{u,v} = \frac{\textit{w}_{u,v}}{{\textit{w}_u}}.
$$
...
If the graph is connected and non-bipartite, the random walk is said to be \textbf{ergodic} and its probability distribution converges to the stationary distribution $\pi \in \mathbb{R}^d$ defined as: $
\pi_u = \frac{\sum_{v\in \mathbf{X}}\textit{w}_{u,v}}{\sum_{u,v \in \mathbf{X}}\textit{w}_{u,v}} = \frac{w_u}{W}
$:$$
\boxed{\lim_{t \to \infty} \mathbb{P}(Y_t = u \mid Y_v = v) = \pi_u, \quad \forall u,v \in \mathbf{X}.}
$$
\end{document}